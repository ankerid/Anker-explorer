...
module.exports = {
    ...
    get_txcount: function (cb) {
      Tx.count({}, function(err, c) {
           return cb(c);
      });
    },

    get_adrcount: function (cb) {
      Address.count({}, function(err, c) {
           return cb(c);
      });
    },

    get_first: function (cb) {
      Tx.findOne().exec(function(err, c) {
           return cb(c.timestamp);
      });
    },
    ...
    find_masternode: function (address, cb) {
        Masternodes.findOne({address: address}, function (err, masternode) {
            if (err) {
                return cb(null);
            } else {
                if (masternode) {
    ...
    get_masternodes: function (cb) {
        Masternodes.find({}, function (err, masternodes) {
            if (err) {
                return cb([]);
            } else {
